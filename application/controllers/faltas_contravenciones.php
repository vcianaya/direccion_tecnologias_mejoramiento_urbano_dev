 ...
 * 
 */
class Faltas_contravenciones extends CI_Controller {
...
//    $this->load->view('footer');
    ...
    function eliminar($value, $row) {
        ...
        $data = $this->db->get_where('faltas_contravenciones', array('faltas_contravenciones.id_faltas' => $row->id_faltas))->row();
        return '<a onClick="if (confirma() == false)
                            return false" href="' . base_url() . 'faltas_contravenciones/borrar_faltas_contravenciones/' . $row->id_faltas . '">
    ...
    function editar($value, $row) {
        $this->db->select('*'); //seleccionamos el id y todo de employer
        $data = $this->db->get_where('faltas_contravenciones', array('faltas_contravenciones.id_faltas' => $row->id_faltas))->row();
        return '<a  href="' . base_url() . 'faltas_contravenciones/edicion_faltas_contravenciones/' . $row->id_faltas . '">
    ...
    function aprobar($value, $row) {
        ...
        return '<a onClick="if (confirma_aprobar() == false)
                                                                return false" href="' . base_url() . 'faltas_contravenciones/aprobar_faltas_contravenciones/' . $row->id_faltas . '">
    ...
    function copiar($value, $row) {
        $this->db->select('*'); //seleccionamos el id y todo de employer
        $data = $this->db->get_where('faltas_contravenciones', array('faltas_contravenciones.id_faltas' => $row->id_faltas))->row();
        return ' <a  href="' . base_url() . 'faltas_contravenciones/copiar_faltas_contravenciones/' . $row->id_faltas . '">
                                                    <img type="image" src="' . base_url() . '/assets/img/copiar.png" height="50px" title="Copiar Formulario">
                                                </a>';
    }

    public function registros_faltas_contravenciones() {
...
// $data["solicitud_compra"] = $this->observatorio_model->lista_operativos();
        ...
        $this->load->view('faltas_contravenciones/registros_faltas_contravenciones', $data);
    }
...
//$e->pos_x;
        ...
        $this->load->view('distritos/distritos_edit', $edit);
