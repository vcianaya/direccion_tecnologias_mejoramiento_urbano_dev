...
//    $this->load->view('footer');
    ...
    function ver_informe_general($value, $row) {
        $this->db->select('*'); //seleccionamos el id y todo de employer
        $data = $this->db->get_where('distribucion', array('distribucion.id' => $row->id))->row();
        return '<a  href="' . base_url() . 'distribucion/ver_informe_imprimir/' . $row->id . '">
                                                    <img type="image" src="' . base_url() . '/assets/img/imprimir.png" height="45px" title="Imprimir" >
                                                </a>';
    }
     public function ver_informe_imprimir() {
        $id = $this->uri->segment(3);
        $data['edit'] = $this->cim_model->ver_informe($id);
        $data['titulo'] = 'Bienvenido de nuevo ' . $this->session->userdata('perfil');
        $this->load->view('header');
        $this->load->view('distribucion/ver_informe', $data);
    }

    public function registro_distribucion() {

        $crud = new grocery_CRUD();
        $crud->set_table('distribucion');
        $crud->columns('Imprimir Informe', 'fecha_registro', 'ficha_nro', 'nombre_apellido','ci', 'nro_celular', 'email', 'nro_licencia_funcionamiento', 
                'nro_pmc', 'patente', 'fecha_autorizacion', 'vigencia','nombre_actividad_economica', 'direccion', 'distrito', 'zona',
                'pos_x', 'pos_y', 'tipo_bien_inmueble', 'producto_distribuye_comercializacion', 'forma_procedimiento_distribucion', 
                'sector_distribucion', 'numero_vehiculos','marca','modelo','color', 'placa_control', 'distintivo', 'descripcion_distintivo',
                'observaciones');
        $crud->add_fields('fecha_registro', 'ficha_nro', 'nombre_apellido','ci', 'nro_celular', 'email', 'nro_licencia_funcionamiento', 
                'nro_pmc', 'patente', 'fecha_autorizacion', 'vigencia','nombre_actividad_economica', 'direccion', 'distrito', 'zona',
                'pos_x', 'pos_y', 'tipo_bien_inmueble', 'producto_distribuye_comercializacion', 'forma_procedimiento_distribucion', 
                'sector_distribucion', 'numero_vehiculos','marca','modelo','color', 'placa_control', 'distintivo', 'descripcion_distintivo',
                'observaciones');
        ...
        $crud->set_relation('tipo_bien_inmueble', 'tipo_bien_inmueble', 'nombre_tipo_bien_inmueble');
        $crud->set_relation('forma_procedimiento_distribucion', 'forma_procedimiento_distribucion', 'nombre_forma_procedimiento_distribucion');
        $crud->set_relation('distrito', 'distrito', 'nombre_distrito');
        $crud->set_relation('zona', 'zona', 'nombre');
        $crud->callback_column('Imprimir Informe', array($this, 'ver_informe_general'));
...
// $data["solicitud_compra"] = $this->observatorio_model->lista_operativos();
        ...
        $this->load->view('distribucion/registro_distribucion', $data);
    }
