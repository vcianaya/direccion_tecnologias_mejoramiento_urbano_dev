 ...
 * 
 */
class Auxilios extends CI_Controller {
    ...
    public function __construct() {
        ...
        $this->load->model('auxilios_model');
...
//    $this->load->view('footer');
    ...
    function eliminar($value, $row) {
        ...
        $data = $this->db->get_where('auxilios', array('auxilios.id_auxilios' => $row->id_auxilios))->row();
        return '<a onClick="if (confirma() == false)
                            return false" href="' . base_url() . 'auxilios/borrar_auxilios/' . $row->id_auxilios . '">
    ...
    function editar($value, $row) {
        $this->db->select('*'); //seleccionamos el id y todo de employer
        $data = $this->db->get_where('auxilios', array('auxilios.id_auxilios' => $row->id_auxilios))->row();
        return '<a  href="' . base_url() . 'auxilios/edicion_auxilios/' . $row->id_auxilios . '">
    ...
    function aprobar($value, $row) {
        ...
        return '<a onClick="if (confirma_aprobar() == false)
                                                                return false" href="' . base_url() . 'auxilios/aprobar_auxilios/' . $row->id_auxilios . '">
    ...
    function copiar($value, $row) {
        $this->db->select('*'); //seleccionamos el id y todo de employer
        $data = $this->db->get_where('auxilios', array('auxilios.id_auxilios' => $row->id_auxilios))->row();
        return ' <a  href="' . base_url() . 'auxilios/copiar_auxilios/' . $row->id_auxilios . '">
                                                    <img type="image" src="' . base_url() . '/assets/img/copiar.png" height="50px" title="Copiar Formulario">
                                                </a>';
    }

    public function registros_auxilios() {

        $usuario = $this->session->userdata('id_usuario');
        $crud = new grocery_CRUD();
// $crud->set_theme('datatables');
        $crud->set_table('auxilios');
        $crud->set_subject('auxilios');
//$crud->set_table('defensoria_na');
        $crud->where('id_usuario', $usuario);
        $crud->where('estado_aprobacion', 0);
        $crud->columns(
                'Eliminar Registro', 'Editar Registro', 'Aprobar Registro', 'Copiar Registro', 'id_auxilios', 'numero_formulario', 'cod_num', 'gestion', 'fecha_hecho', 'hora_hecho', 'mes', 'departamento', 'provincia', 'municipio', 'localidad', 'distrito', 'zona', 'lugar_hecho', 'pos_x', 'pos_y', 'epis', 'nombre_auxiliador', 'ci', 'nacido_en', 'nacionalidad', 'edad', 'genero', 'telefono_referencia', 'temperancia', 'personas_capacidades_diferentes', 'diferentes_auxilios', 'remision_caso', 'servicio_destacado', 'nombre_policia');
        ...
        $crud->set_relation('diferentes_auxilios', 'diferentes_auxilios', 'nombre_diferentes_auxilios');
        ...
        $data = array(
            "provincias" => $this->auxilios_model->getProvincias(),
            "gestion" => $this->auxilios_model->gestion(),
            "mes" => $this->auxilios_model->mes(),
            "departamento" => $this->auxilios_model->departamento(),
            "provincia" => $this->auxilios_model->provincia(),
            "municipio" => $this->auxilios_model->municipio(),
            "localidad" => $this->auxilios_model->localidad(),
            "epis" => $this->auxilios_model->epis(),
            "nacionalidad" => $this->auxilios_model->nacionalidad(),
            //  "denuncia" => $this->faltas_contravenciones_model->denuncia(),
            "genero" => $this->auxilios_model->genero(),
            "temperancia" => $this->auxilios_model->temperancia(),
            ...
            "servicio" => $this->auxilios_model->servicio(),
            //   "arrestado" => $this->faltas_contravenciones_model->arrestado(),
            // "sancion" => $this->faltas_contravenciones_model->sancion(),
            "diferentes_auxilios" => $this->auxilios_model->diferentes_auxilios(),
            "remision_caso" => $this->auxilios_model->remision_caso(),
            ...
            "servicio_destacado" => $this->auxilios_model->servicio_destacado(),
            "auxilios" => $this->auxilios_model->auxilios()
        );
...
// $data["solicitud_compra"] = $this->observatorio_model->lista_operativos();
        ...
        $this->load->view('auxilios/registros_auxilios', $data);
    }

    public function guardar_auxilios() {
        ...
        $guardar_auxilios = array(
            ...
            'nombre_auxiliador' => $this->input->post('nombre_auxiliador'),
            ...
            'personas_capacidades_diferentes' => $this->input->post('personas_capacidades_diferentes'),
            'diferentes_auxilios' => $this->input->post('diferentes_auxilios'),
...
//  $id_distrito = $this->input->post('provincias');
        ...
        $this->auxilios_model->guardar_auxilios($guardar_auxilios);

        redirect(base_url() . 'auxilios/registros_auxilios');
    }

    public function update_auxilios() {
        ...
        $id_auxilios = $this->input->post('id_auxilios');
//  $id_distrito = $this->input->post('provincias');

        $this->auxilios_model->update_auxilios($guardar_auxilios, $id_auxilios);

        redirect(base_url() . 'auxilios/registros_auxilios');
    }

    public function edicion_auxilios() {
        $data = array(
            "provincias" => $this->auxilios_model->getProvincias(),
            "gestion" => $this->auxilios_model->gestion(),
            "mes" => $this->auxilios_model->mes(),
        ...
        $id_auxilio = $this->uri->segment(3);

        $edit = $this->auxilios_model->auxilios_edit($id_auxilio);
        foreach ($edit as $e):
            //$e->pos_x;
        endforeach;


        $this->load->view('distritos/distritos_edit_auxilios', $edit);

        $id_auxilios = $this->uri->segment(3);

        $data['datos'] = $this->mapa_model->get_markers_general_monitoreo_ubicacion();
        $data['edit'] = $this->auxilios_model->auxilios_edit($id_auxilios);
        $data['map'] = $this->googlemaps->create_map();
...
// $data["solicitud_compra"] = $this->observatorio_model->lista_operativos();
        $data['titulo'] = 'Bienvenido de nuevo ' . $this->session->userdata('perfil');
        $this->load->view('header');
        $this->load->view('auxilios/editar_auxilios', $data);
    }

    public function copiar_auxilios() {
        $data = array(
            "provincias" => $this->auxilios_model->getProvincias(),
            "gestion" => $this->auxilios_model->gestion(),
            "mes" => $this->auxilios_model->mes(),
        ...
        $edit = $this->auxilios_model->auxilios_edit($id_auxilios);
        foreach ($edit as $e):
            //$e->pos_x;
        endforeach;
        ...
        $data['datos'] = $this->mapa_model->get_markers_general_monitoreo_ubicacion();
        $data['edit'] = $this->auxilios_model->auxilios_edit($id_faltas);
        $data['map'] = $this->googlemaps->create_map();
...
// $data["solicitud_compra"] = $this->observatorio_model->lista_operativos();
        $data['titulo'] = 'Bienvenido de nuevo ' . $this->session->userdata('perfil');
        $this->load->view('header');
        $this->load->view('auxilios/copiar_auxilios', $data);
    }
...
//  $id_distrito = $this->input->post('provincias');
    ...
    public function aprobar_auxilios() {
        $id = $this->uri->segment(3);
        $this->auxilios_model->aprobar_auxilios($id);

        redirect(base_url() . 'auxilios/registros_auxilios');
    }

    public function borrar_auxilios() {
        $id = $this->uri->segment(3);
        $this->auxilios_model->eliminar_auxilios($id);
        redirect(base_url() . 'auxilios/registros_auxilios');
    }
