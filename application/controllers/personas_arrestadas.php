 ...
 * 
 */
class Personas_arrestadas extends CI_Controller {
    ...
    public function __construct() {
        ...
        $this->load->model('personas_arrestadas_model');
...
//    $this->load->view('footer');
    ...
    function eliminar($value, $row) {
        ...
        $data = $this->db->get_where('personas_arrestadas', array('personas_arrestadas.id_arrestadas' => $row->id_arrestadas))->row();
        return '<a onClick="if (confirma() == false)
                            return false" href="' . base_url() . 'personas_arrestadas/borrar_personas_arrestadas/' . $row->id_arrestadas . '">
    ...
    function editar($value, $row) {
        $this->db->select('*'); //seleccionamos el id y todo de employer
        $data = $this->db->get_where('personas_arrestadas', array('personas_arrestadas.id_arrestadas' => $row->id_arrestadas))->row();
        return '<a  href="' . base_url() . 'personas_arrestadas/edicion_personas_arrestadas/' . $row->id_arrestadas . '">
    ...
    function aprobar($value, $row) {
        ...
        return '<a onClick="if (confirma_aprobar() == false)
                                                                return false" href="' . base_url() . 'personas_arrestadas/aprobar_personas_arrestadas/' . $row->id_arrestadas . '">
    ...
    function copiar($value, $row) {
        $this->db->select('*'); //seleccionamos el id y todo de employer
        $data = $this->db->get_where('personas_arrestadas', array('personas_arrestadas.id_arrestadas' => $row->id_arrestadas))->row();
        return ' <a  href="' . base_url() . 'personas_arrestadas/copiar_personas_arrestadas/' . $row->id_arrestadas . '">
                                                    <img type="image" src="' . base_url() . '/assets/img/copiar.png" height="50px" title="Copiar Formulario">
                                                </a>';
    }

    public function registros_personas_arrestadas() {

        $usuario = $this->session->userdata('id_usuario');
        $crud = new grocery_CRUD();
// $crud->set_theme('datatables');
        $crud->set_table('personas_arrestadas');
        $crud->set_subject('personas_arrestadas');
//$crud->set_table('defensoria_na');
        $crud->where('id_usuario', $usuario);
        $crud->where('estado_aprobacion', 0);
        $crud->columns(
                'Eliminar Registro', 'Editar Registro', 'Aprobar Registro', 'Copiar Registro', 'id_arrestadas', 'numero_formulario', 'cod_num', 'gestion', 'fecha_hecho', 'hora_hecho', 'mes', 'departamento', 'provincia', 'municipio', 'localidad', 'distrito', 'zona', 'lugar_hecho', 'pos_x', 'pos_y', 'epis', 'nombre_arrestado', 'ci', 'nacido_en', 'genero', 'telefono_referencia', 'temperancia', 'causas_arresto', 'naturaleza_hecho', 'arma_instrumento', 'remision_caso', 'servicio_destacado', 'nombre_policia', 'pertenencias_arrestado');
        ...
        //   $crud->set_relation('nacionalidad', 'nacionalidad', 'nombre_nacionalidad');
        $crud->set_relation('genero', 'genero', 'nombre_genero');

        $crud->set_relation('temperancia', 'temperancia', 'nombre_temperancia');

        $crud->set_relation('naturaleza_hecho', 'naturaleza_hecho', 'nombre_naturaleza_hecho');
        ...
        $data = array(
            "provincias" => $this->personas_arrestadas_model->getProvincias(),
            "gestion" => $this->personas_arrestadas_model->gestion(),
            "mes" => $this->personas_arrestadas_model->mes(),
            "departamento" => $this->personas_arrestadas_model->departamento(),
            "provincia" => $this->personas_arrestadas_model->provincia(),
            "municipio" => $this->personas_arrestadas_model->municipio(),
            "localidad" => $this->personas_arrestadas_model->localidad(),
            "epis" => $this->personas_arrestadas_model->epis(),
            "nacionalidad" => $this->personas_arrestadas_model->nacionalidad(),
            //  "denuncia" => $this->faltas_contravenciones_model->denuncia(),
            "genero" => $this->personas_arrestadas_model->genero(),
            "temperancia" => $this->personas_arrestadas_model->temperancia(),
            ...
            "servicio" => $this->personas_arrestadas_model->servicio(),
            //   "arrestado" => $this->faltas_contravenciones_model->arrestado(),
            // "sancion" => $this->faltas_contravenciones_model->sancion(),
            "naturaleza_hecho" => $this->personas_arrestadas_model->naturaleza_hecho(),
            "remision_caso" => $this->personas_arrestadas_model->remision_caso(),
            ...
            "servicio_destacado" => $this->personas_arrestadas_model->servicio_destacado()
        );
...
// $data["solicitud_compra"] = $this->observatorio_model->lista_operativos();
        ...
        $this->load->view('personas_arrestadas/registros_personas_arrestadas', $data);
    }

    public function guardar_personas_arrestadas() {
        ...
        $guardar_personas_arrestadas = array(
            ...
            'nombre_arrestado' => $this->input->post('nombre_arrestado'),
            //   'nombre_auxiliador' => $this->input->post('nombre_victima'),
            'ci' => $this->input->post('ci'),
            'nacido_en' => $this->input->post('nacido_en'),
            //    'nacionalidad' => $this->input->post('nacionalidad'),
            // 'edad' => $this->input->post('edad'),
            'genero' => $this->input->post('genero'),
            'telefono_referencia' => $this->input->post('telefono_referencia'),
            'temperancia' => $this->input->post('temperancia'),
            'causas_arresto' => $this->input->post('causas_arresto'),
            'naturaleza_hecho' => $this->input->post('naturaleza_hecho'),
            'arma_instrumento' => $this->input->post('arma_instrumento'),
            'remision_caso' => $this->input->post('remision_caso'),
            'servicio_destacado' => $this->input->post('servicio_destacado'),
            'nombre_policia' => $this->input->post('nombre_policia'),
            'pertenencias_arrestado' => $this->input->post('pertenencias_arrestado'),
...
//  $id_distrito = $this->input->post('provincias');
        ...
        $this->personas_arrestadas_model->guardar_personas_arrestadas($guardar_personas_arrestadas);

        redirect(base_url() . 'personas_arrestadas/registros_personas_arrestadas');
    }

    public function update_personas_arrestadas() {
        ...
        $id_arrestadas = $this->input->post('id_arrestadas');
//  $id_distrito = $this->input->post('provincias');

        $this->personas_arrestadas_model->update_personas_arrestadas($guardar_personas_arrestadas, $id_arrestadas);

        redirect(base_url() . 'personas_arrestadas/registros_personas_arrestadas');
    }

    public function edicion_personas_arrestadas() {
        $data = array(
            "provincias" => $this->personas_arrestadas_model->getProvincias(),
            "gestion" => $this->personas_arrestadas_model->gestion(),
            "mes" => $this->personas_arrestadas_model->mes(),
            ...
            //"recurso_apelacion" => $this->faltas_contravenciones_model->recurso_apelacion(),
            "servicio_destacado" => $this->personas_arrestadas_model->servicio_destacado(),
            "personas_arrestadas" => $this->personas_arrestadas_model->personas_arrestadas()
        );
        $id_arrestadas = $this->uri->segment(3);

        $edit = $this->personas_arrestadas_model->personas_arrestadas_edit($id_arrestadas);
        foreach ($edit as $e):
            //$e->pos_x;
        endforeach;
        $this->load->view('distritos/distritos_edit_personas_arrestadas', $edit);
//       $this->googlemaps->add_polygon($cam1);
//$marker ['icon'] = base_url().'imagenes/'.$fila->imagen;
//si queremos que se pueda arrastrar el marker
//$marker['draggable'] = TRUE;
...
//tendremos disponible en la vista mapa_view con el array data
        ...
        $data['datos'] = $this->mapa_model->get_markers_general_monitoreo_ubicacion();
        $data['edit'] = $this->personas_arrestadas_model->personas_arrestadas_edit($id_arrestadas);
        $data['map'] = $this->googlemaps->create_map();
...
// $data["solicitud_compra"] = $this->observatorio_model->lista_operativos();
        $data['titulo'] = 'Bienvenido de nuevo ' . $this->session->userdata('perfil');
        $this->load->view('header');
        $this->load->view('personas_arrestadas/editar_personas_arrestadas', $data);
    }

    public function copiar_personas_arrestadas() {
        ...
        $id_personas_arrestadas = $this->uri->segment(3);
...
// $data["solicitud_compra"] = $this->observatorio_model->lista_operativos();
        $data['titulo'] = 'Bienvenido de nuevo ' . $this->session->userdata('perfil');
        $this->load->view('header');
        $this->load->view('personas_arrestadas/copiar_personas_arrestadas', $data);
    }
    ...
    public function editar_faltas_contravenciones() {
        ...
        $date = strtotime($this->input->post('fecha_hecho'));
        $dia = date("d", $date);
        $faltas_contravenciones = $this->personas_arrestadas_model->faltas_contravenciones_edit($id_faltas);
...
//  $id_distrito = $this->input->post('provincias');
    ...
    public function aprobar_personas_arrestadas() {
        $id = $this->uri->segment(3);
        $this->personas_arrestadas_model->aprobar_personas_arrestadas($id);

        redirect(base_url() . 'personas_arrestadas/registros_personas_arrestadas');
    }

    public function borrar_personas_arrestadas() {
        $id = $this->uri->segment(3);
        $this->personas_arrestadas_model->eliminar_personas_arrestadas($id);
        redirect(base_url() . 'personas_arrestadas/registros_personas_arrestadas');
    }
