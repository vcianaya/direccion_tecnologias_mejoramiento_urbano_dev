...
//    $this->load->view('footer');
    ...
    public function index() {
        $data["notas"] = $this->mapa_model->notas();
        $data["jefaturas"] = $this->mapa_model->jefaturas();
        $data['titulo'] = 'Bienvenido de nuevo ' . $this->session->userdata('perfil');
        $this->load->view('header');
        $this->load->view('notas/seleccion_nota', $data);
    }

    public function registros_cites() {
        $notas = $this->input->post('notas');
        $jefaturas = $this->input->post('jefaturas');

        $data["notas_consulta"] = $this->mapa_model->notas_consulta($notas);
        $data["jefaturas_consulta"] = $this->mapa_model->jefaturas_consulta($jefaturas);
        $data['titulo'] = 'Bienvenido de nuevo ' . $this->session->userdata('perfil');
        $this->load->view('header');
        $this->load->view('notas/registros_cites', $data);
    }
    ...
    public function guardar_notas() {
        ...
        $guardar_notas = array(
            ...
            'donde_se_envia' => $this->input->post('donde_se_envia'),
            'de_quien_recibe' => $this->input->post('de_quien_recibe'),
            'referencia' => $this->input->post('referencia'),
            'fecha' => $this->input->post('fecha')
           
        );
        $this->mapa_model->guardar_notas($guardar_notas);
        $this->session->set_flashdata('correcto', 'Registrado correctamente! Su Numero de Cite es: '.$this->input->post('cite'));
    ...
    public function registro_protocolo() {
        ...
        // $crud->columns('*', 'mapas');
        $crud->edit_fields('distrito', 'direccion', 'entidad', 'motivo', 'trabajo_realizado', 'realizado_por', 'nombre_propietario', 'cargo_propietario', 'imagen_soporte', 'fecha', 'hora', 'pos_x', 'pos_y');
