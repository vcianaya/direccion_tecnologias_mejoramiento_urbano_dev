 ...
 * 
 */
class Felcc extends CI_Controller {
...
//    $this->load->view('footer');
    ...
    public function registros_felcc() {

        $crud = new grocery_CRUD();
        $crud->set_table('felcc');
        ...
        $crud->columns('Eliminar Registro', 'Editar Registro', 'Aprobar Registro', 'Copiar Registro', 'id_felcc', 'numero_formulario', 'cod_num', 'gestion', 'fecha_hecho', 'hora_hecho', 'mes', 'departamento', 'provincia', 'municipio', 'localidad', 'epis', 'division', 'n_registro', 'delitos_hecho', 'nombre_victima', 'edad_victima', 'sexo_victima', 'ci_victima', 'estado_civil_victima', 'ocupacion_victima', 'domicilio_victima', 'temperancia_victima', 'nacionalidad_victima', 'nombre_denunciado', 'nombre_complices', 'edad_denunciado', 'sexo_denunciado', 'ci_denunciado', 'estado_civil_denunciado', 'ocupacion_denunciado', 'domicilio_denunciado', 'temperancia_denunciado', 'nacionalidad_denunciado', 'alias', 'arrestado', 'distrito', 'zona', 'lugar_hecho', 'lugar_especifico', 'causas_hecho', 'objetos_robados', 'medios_utilizados', 'instrumento_utilizado', 'caso_n', 'asignado', 'estado_caso', 'detalle_caso');
        ...
        $crud->set_relation('delitos_hecho', 'delito', 'nombre_delito');
        ...
        $crud->set_relation('sexo_denunciado', 'genero_infractor', 'nombre_genero_infractor');

        $crud->set_relation('estado_civil_victima', 'estado_civil', 'nombre_estado_civil');
        $crud->set_relation('estado_civil_denunciado', 'estado_civil_denunciado', 'nombre_estado_civil_denunciado');
        $crud->set_relation('ocupacion_victima', 'ocupacion', 'nombre_ocupacion');
        ...
        $crud->set_relation('nacionalidad_denunciado', 'nacionalidad', 'nombre_nacionalidad');
        //  $crud->set_relation('tipo_vehiculo', 'tipo_vehiculo', 'nombre_tipo_vehiculo');
        $crud->set_relation('temperancia_denunciado', 'temperancia', 'nombre_temperancia');
...
// $data["solicitud_compra"] = $this->observatorio_model->lista_operativos();
        ...
        $this->load->view('felcc/registros_felcc', $data);
    }

    public function guardar_felcc() {
...
//  $id_distrito = $this->input->post('provincias');
        ...
        $this->faltas_contravenciones_model->guardar_felcc($guardar_felcc);
...
//       $this->googlemaps->add_polygon($cam1);fedicion_felcc
...
// $data["solicitud_compra"] = $this->observatorio_model->lista_operativos();
        $data['titulo'] = 'Bienvenido de nuevo ' . $this->session->userdata('perfil');
        $this->load->view('header');
        $this->load->view('felcc/copiar_felcc', $data);
    }
...
//  $id_distrito = $this->input->post('provincias');
    ...
    public function excel_felcc() {
        $this->load->model('faltas_contravenciones_model');
        to_excel($this->faltas_contravenciones_model->get_felcc(), "felcc");
    }

    public function felcc_seleccion_reporte() {
        $data = array(
            "division" => $this->felcc_model->lista_divisiones(),
            "mes" => $this->felcc_model->mes()
        );

        $data['titulo'] = 'Bienvenido de nuevo ' . $this->session->userdata('perfil');
        $this->load->view('header');
        $this->load->view('felcc/felcc_seleccion_reporte', $data);
    }

    public function reporte_felcc_general() {
...
//$data['map'] = $this->googlemaps->create_map();
        ...
        $markers = $this->felcc_model->get_markers_general($leyenda, $division, $mes);
        ...
        if (!$markers) {
            ...
            foreach ($markers as $info_marker) {
                ...
                $marker ['infowindow_content'] = '  <strong>ID: </strong> ' . $info_marker->id_felcc .
                        '<br/> <strong>Division: </strong>' . $info_marker->nombre_division .
                        ' <br/> <strong>Delito: </strong> ' . $info_marker->nombre_delito .
                        ' <br/> <strong>Fecha Denuncia: </strong> ' . $info_marker->fecha_denuncia .
                        ' <br/> <strong>Fecha Hecho: </strong> ' . $info_marker->fecha_hecho .
                        ' <br/> <strong>Hora Hecho: </strong> ' . $info_marker->hora_hecho;
                $marker['id'] = $info_marker->id_felcc;
                $this->googlemaps->add_marker($marker);
                //$marker ['icon'] = base_url().'imagenes/'.$fila->imagen;
                //si queremos que se pueda arrastrar el marker
                //$marker['draggable'] = TRUE;
        ...
        //creamos el mapa y lo asignamos a map que lo 
        //tendremos disponible en la vista mapa_view con el array data
        $data['datos'] = $this->felcc_model->get_markers_general($leyenda, $division, $mes);
        $data['map'] = $this->googlemaps->create_map();
        $this->load->library('highcharts');


        $tip1 = $this->felcc_model->tip1();
        $tip2 = $this->felcc_model->tip2();
        $tip3 = $this->felcc_model->tip3();
        $tip4 = $this->felcc_model->tip4();
        $tip5 = $this->felcc_model->tip5();
        $tip6 = $this->felcc_model->tip6();
        $tip7 = $this->felcc_model->tip7();
        $tip8 = $this->felcc_model->tip8();
        $tip9 = $this->felcc_model->tip9();
        $tip10 = $this->felcc_model->tip10();


        // $tipTotal = $this->observatorio_model->tipTotal_2();

        $serie['data'] = array(
            array('CORRUPCION PUBLICA', $tip1),
            array('CRIMEN ORGANIZADO ', $tip2),
            array('ECONOMICO FINANCIERO  ', $tip3),
            array('HOMICIDIOS  ', $tip4),
            array('MENORES Y FAMILIA  ', $tip5),                                                                                                                                                                                                                                                                                                                                                                           
            array('OPERACIONES ESPECIALES ', $tip6),
            array('PERSONAS  ', $tip7),
            array('PROPIEDADES  ', $tip8),
            array('TRATA Y TRAFICO ', $tip9),
            array('UNIDAD DE SOLUCION TEMPRANA ', $tip10)
        );
        $callback = "function() { return '<b>'+ this.point.name +'</b>: '+ this.y +' '}";

        @$tool->formatter = $callback;
        @$plot->pie->dataLabels->formatter = $callback;
        $this->highcharts->set_title('Grafico de Felcc General', 'Por Division ');
        $this->highcharts
                ->set_type('pie')
                ->set_serie($serie)
                ->set_tooltip($tool)
        ...
        $op1 = $this->felcc_model->tip_general_1($mes, $division);
        $op2 = $this->felcc_model->tip_general_2($mes, $division);
        $op3 = $this->felcc_model->tip_general_3($mes, $division);
        $op4 = $this->felcc_model->tip_general_4($mes, $division);
        $op5 = $this->felcc_model->tip_general_5($mes, $division);
        $op6 = $this->felcc_model->tip_general_6($mes, $division);
        $op7 = $this->felcc_model->tip_general_7($mes, $division);
        $op8 = $this->felcc_model->tip_general_8($mes, $division);
        $op9 = $this->felcc_model->tip_general_9($mes, $division);
        $op10 = $this->felcc_model->tip_general_10($mes, $division);
        $op11 = $this->felcc_model->tip_general_11($mes, $division);
        $op12 = $this->felcc_model->tip_general_12($mes, $division);
        $op13 = $this->felcc_model->tip_general_13($mes, $division);

        //   $tipTotal = $this->felcc_model->tipTotal();

        $serie['data'] = array(
            array('00:01 - 03:00', $op1),
        ...
        @$tool->formatter = $callback;
        @$plot->pie->dataLabels->formatter = $callback;
        $this->highcharts->set_title('Grafico de Felcc', 'Por Horas ');
        $this->highcharts
                ->set_type('pie')
                ->set_serie($serie)
                ->set_tooltip($tool)
        ...
        $tipd1 = $this->felcc_model->tipd1($mes, $division);
        $tipd2 = $this->felcc_model->tipd2($mes, $division);
        $tipd3 = $this->felcc_model->tipd3($mes, $division);
        $tipd4 = $this->felcc_model->tipd4($mes, $division);
        $tipd5 = $this->felcc_model->tipd5($mes, $division);
        $tipd6 = $this->felcc_model->tipd6($mes, $division);
        $tipd7 = $this->felcc_model->tipd7($mes, $division);
        $tipd8 = $this->felcc_model->tipd8($mes, $division);
        $tipd9 = $this->felcc_model->tipd9($mes, $division);
        $tipd10 = $this->felcc_model->tipd10($mes, $division);
        $tipd11 = $this->felcc_model->tipd11($mes, $division);
        $tipd12 = $this->felcc_model->tipd12($mes, $division);
        $tipd13 = $this->felcc_model->tipd13($mes, $division);
        $tipd14 = $this->felcc_model->tipd14($mes, $division);
        $tipd15 = $this->felcc_model->tipd15($mes, $division);
        ...
        $serie['data'] = array(
            ...
            array('FUERA MUNICIPIO DEL ALTO', $tipd15)
        );
        $callback = "function() { return '<b>'+ this.point.name +'</b>: '+ this.y +' '}";
        ...
        $tipdg1 = $this->felcc_model->tipg1($mes, $division);
        $tipdg2 = $this->felcc_model->tipg2($mes, $division);
        $tipdg3 = $this->felcc_model->tipg3($mes, $division);
        ...
        $tipq1 = $this->felcc_model->tipa1($mes, $division);
        $tipq2 = $this->felcc_model->tipa2($mes, $division);
        $tipq3 = $this->felcc_model->tipa3($mes, $division);
        $tipq4 = $this->felcc_model->tipa4($mes, $division);
        $tipq5 = $this->felcc_model->tipa5($mes, $division);
        $tipq6 = $this->felcc_model->tipa6($mes, $division);
        $tipq7 = $this->felcc_model->tipa7($mes, $division);
        $tipq8 = $this->felcc_model->tipa8($mes, $division);
        $tipq9 = $this->felcc_model->tipa9($mes, $division);
        $tipq10 = $this->felcc_model->tipa10($mes, $division);
        $tipq11 = $this->felcc_model->tipa11($mes, $division);
        $tipq12 = $this->felcc_model->tipa12($mes, $division);
        $tipq13 = $this->felcc_model->tipa13($mes, $division);
        $tipq14 = $this->felcc_model->tipa14($mes, $division);
        ...
        $tipq1 = $this->felcc_model->tipme1($mes, $division);
        $tipq2 = $this->felcc_model->tipme2($mes, $division);
        $tipq3 = $this->felcc_model->tipme3($mes, $division);
        $tipq4 = $this->felcc_model->tipme4($mes, $division);
        $tipq5 = $this->felcc_model->tipme5($mes, $division);
        $tipq6 = $this->felcc_model->tipme6($mes, $division);
        $tipq7 = $this->felcc_model->tipme7($mes, $division);
        $tipq8 = $this->felcc_model->tipme8($mes, $division);
        $tipq9 = $this->felcc_model->tipme9($mes, $division);
        $tipq10 = $this->felcc_model->tipme10($mes, $division);
        $tipq11 = $this->felcc_model->tipme11($mes, $division);
        $tipq12 = $this->felcc_model->tipme12($mes, $division);
        ...
        $data['titulo'] = 'Bienvenido de nuevo ' . $this->session->userdata('perfil');
        $this->load->view('header');
        $this->load->view('felcc/mapa_view_general', $data);
    }
