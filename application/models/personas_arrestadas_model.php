...
class Personas_arrestadas_model extends CI_Model {
    ...
    function update_personas_arrestadas($guardar_personas_arrestadas, $id_arrestadas) {
        $this->db->where('id_arrestadas', $id_arrestadas);
        //  $this->db->set('estado_aprobacion', 1);
        return $this->db->update('personas_arrestadas', $guardar_personas_arrestadas);
    }
    ...
    public function personas_arrestadas() {
        $this->db->select('*');
        $this->db->where("id_usuario", $this->session->userdata('id_usuario'));
        $this->db->where("estado_aprobacion", 0);
        // $this->db->where("id_faltas", $id_faltas);

        $this->db->join('mes', 'mes.id = personas_arrestadas.mes');

        $this->db->join('epis', 'epis.id = personas_arrestadas.epis');

       // $this->db->join('diferentes_auxilios', 'diferentes_auxilios.id = auxilios.diferentes_auxilios');

        $this->db->from('personas_arrestadas');
    ...
    public function personas_arrestadas_edit($id_arrestadas) {
        $this->db->select('*');
        //   $this->db->where("id_usuario", $this->session->userdata('id_usuario'));
        $this->db->where("id_arrestadas", $id_arrestadas);
        $this->db->join('gestion', 'gestion.id = personas_arrestadas.gestion');
        $this->db->join('mes', 'mes.id = personas_arrestadas.mes');
        $this->db->join('departamento', 'departamento.id = personas_arrestadas.departamento');
        $this->db->join('provincia', 'provincia.id = personas_arrestadas.provincia');
        $this->db->join('municipio', 'municipio.id = personas_arrestadas.municipio');
        $this->db->join('localidad', 'localidad.id = personas_arrestadas.localidad');
        $this->db->join('epis', 'epis.id = personas_arrestadas.epis');
        $this->db->join('distrito', 'distrito.id = personas_arrestadas.distrito');
        $this->db->join('zona', 'zona.id = personas_arrestadas.zona');
   //     $this->db->join('denuncia', 'denuncia.id = auxilios.denuncia');
    ...
    //    $this->db->join('nacionalidad', 'nacionalidad.id = auxilios.nacionalidad');
        //   $this->db->join('nacionalidad_infractor', 'nacionalidad_infractor.id = faltas_contravenciones.nacionalidad_infractor');
        $this->db->join('genero', 'genero.id = personas_arrestadas.genero');
        //     $this->db->join('genero_infractor', 'genero_infractor.id = faltas_contravenciones.sexo_infractor');
        $this->db->join('temperancia', 'temperancia.id = personas_arrestadas.temperancia');
            $this->db->join('naturaleza_hecho', 'naturaleza_hecho.id = personas_arrestadas.naturaleza_hecho');
    ...
    //    $this->db->join('diferentes_auxilios', 'diferentes_auxilios.id = personas_arrestadas.diferentes_auxilios');
        $this->db->join('remision_caso', 'remision_caso.id = personas_arrestadas.remision_caso');
        //       $this->db->join('termino_prueba', 'termino_prueba.id = faltas_contravenciones.termino_prueba');
    //    $this->db->join('recurso_apelacion', 'recurso_apelacion.id = personas_arrestadas.recurso_apelacion');
        $this->db->join('servicio_destacado', 'servicio_destacado.id = personas_arrestadas.servicio_destacado');
    ...
    public function guardar_personas_arrestadas($guardar_personas_arrestadas) {
        $this->db->insert('personas_arrestadas', $guardar_personas_arrestadas);
    }
...
//// FELCCCCCCC
    ...
    function eliminar_personas_arrestadas($id) {
        $this->db->where('id_arrestadas', $id);
        return $this->db->delete('personas_arrestadas');
    }

    function aprobar_personas_arrestadas($id) {
        $this->db->where('id_arrestadas', $id);
        $this->db->set('estado_aprobacion', 1);
        return $this->db->update('personas_arrestadas');
    }
