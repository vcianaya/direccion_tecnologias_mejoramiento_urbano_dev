...
class Faltas_contravenciones_model extends CI_Model {
...
//// FELCCCCCCC
    ...
    public function get_areas() {
        $markers = $this->db->join('area', 'area.id = luminarias.area');
    ...
    public function get_areas2($area) {
        $markers = $this->db->where('area.id', $area);
        $markers = $this->db->get('area');
        if ($markers->num_rows() > 0) {
            return $markers->result();
        }
    }

    public function get_luminarias($luminaria) {
        $markers = $this->db->where('id', $luminaria);
        $markers = $this->db->get('luminarias');
        if ($markers->num_rows() > 0) {
            return $markers->result();
        }
