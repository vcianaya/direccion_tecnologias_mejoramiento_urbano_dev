...
class Auxilios_model extends CI_Model {
    ...
    function update_auxilios($guardar_auxilios, $id_auxilios) {
        $this->db->where('id_auxilios', $id_auxilios);
        //  $this->db->set('estado_aprobacion', 1);
        return $this->db->update('auxilios', $guardar_auxilios);
    }
    ...
    public function auxilios_edit($id_auxilio) {
        $this->db->select('*');
        //   $this->db->where("id_usuario", $this->session->userdata('id_usuario'));
        $this->db->where("id_auxilios", $id_auxilio);
        $this->db->join('gestion', 'gestion.id = auxilios.gestion');
        $this->db->join('mes', 'mes.id = auxilios.mes');
        $this->db->join('departamento', 'departamento.id = auxilios.departamento');
        $this->db->join('provincia', 'provincia.id = auxilios.provincia');
        $this->db->join('municipio', 'municipio.id = auxilios.municipio');
        $this->db->join('localidad', 'localidad.id = auxilios.localidad');
        $this->db->join('epis', 'epis.id = auxilios.epis');
        $this->db->join('distrito', 'distrito.id = auxilios.distrito');
        $this->db->join('zona', 'zona.id = auxilios.zona');
   //     $this->db->join('denuncia', 'denuncia.id = auxilios.denuncia');
        ...
        $this->db->join('genero', 'genero.id = auxilios.genero');
        //     $this->db->join('genero_infractor', 'genero_infractor.id = faltas_contravenciones.sexo_infractor');
        $this->db->join('temperancia', 'temperancia.id = auxilios.temperancia');
        ...
        $this->db->join('diferentes_auxilios', 'diferentes_auxilios.id = auxilios.diferentes_auxilios');
        $this->db->join('remision_caso', 'remision_caso.id = auxilios.remision_caso');
    ...
    //    $this->db->join('recurso_apelacion', 'recurso_apelacion.id = auxilios.recurso_apelacion');
        $this->db->join('servicio_destacado', 'servicio_destacado.id = auxilios.servicio_destacado');
    ...
    public function guardar_auxilios($guardar_auxilios) {
        $this->db->insert('auxilios', $guardar_auxilios);
    }
